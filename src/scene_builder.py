"""
Scene Builder v5.2 - Character Behavior Model (фаза 2)
Наследование правил + Валидатор + PreferenceResolver: характер персонажа
взвешивает действия/лицо/позу; поза выбирается всегда (закрытие дыры).
Запись в composite-поля scene (pose_*/face_*) — через hasattr (defensive).
"""
import random
...
class SceneBuilder:
    ...
    # Маппинг имя_файла_библиотеки -> имя_слота в модели характера
    _POSE_FILE2SLOT = {'base': 'base', 'head': 'head', 'arms': 'arms', 'legs': 'legs'}
    ...
    def __init__(self, library: PromptLibrary, scene_rules: dict, character_profile: dict, location_types: dict, generation_weights: dict | None = None):
        ...
        # ── Характер персонажа (Character Behavior Model, фаза 2) ──
        self.personality = CharacterPersonality.from_dict(
            self.full_profile.get('personality') if self.full_profile else None)
        self._personality_neutral = self.personality.is_neutral()
        self._slot_index = None  # ленивый индекс тег→слот для позы/лица
    ...
    # ═══════════════════════════════════════════════════════════════
    # PREFERENCE RESOLVER (фаза 2) — чистая логика весов по слотам
    # ═══════════════════════════════════════════════════════════════
    @staticmethod
    ...
    def _choose_slot_tag(slot: str, mode, slot_index: dict,
                         prefers: list, avoid: list) -> str:
        """Resolver одного слота: final_weight = char × world_boost.
        prefers/avoid — плоские списки тегов из мира (prefers_poses и т.п.),
        уже отфильтрованные по принадлежности к этому слоту вызывающим кодом
        не требуется: здесь буст применяется, только если тег реально в слоте."""
        cands = slot_index.get(slot, [])
        if not cands:
            return ""
        weights = []
        for t in cands:
            w = SceneBuilder._char_weight(mode, slot, t)
            if t in prefers:
                w *= SceneBuilder._PREFER_BOOST
            if t in avoid:
                w *= SceneBuilder._AVOID_PENALTY
            weights.append(w)
        total = sum(weights)
        if total <= 0.0:  # весь слот забанен характером/миром — редкий кейс
            return random.choice(cands)
        return random.choices(cands, weights=weights, k=1)[0]

    @staticmethod
    def _apply_pose(scene: Scene, mode, slot_index: dict,
                    prefers: list, avoid: list) -> None:
        """Заполняет composite-позу (base/head/arms/legs). Всегда.
        Если у scene нет composite-полей (старый scene.py) — тихо no-op."""
        chosen = {s: SceneBuilder._choose_slot_tag(s, mode, slot_index, prefers, avoid)
                  for s in ('base', 'head', 'arms', 'legs')}
        if hasattr(scene, 'pose_base'):
            scene.pose_base = chosen['base']
            scene.pose_head = chosen['head']
            scene.pose_arms = chosen['arms']
            scene.pose_legs = chosen['legs']

    @staticmethod
    def _apply_face(scene: Scene, mode, slot_index: dict,
                    prefers: list, avoid: list, neutral: bool) -> None:
        """Лицо. neutral=True (нет блока personality) => старое поведение:
        плоский scene.expression из prefers. Иначе => composite face по весам.
        Если у scene нет composite-полей — деградация в плоский expression."""
        if neutral:
            if prefers:
                scene.expression = random.choice(prefers)
            return
        chosen = {s: SceneBuilder._choose_slot_tag(s, mode, slot_index, prefers, avoid)
                  for s in ('mood', 'eyes', 'mouth')}
        if hasattr(scene, 'face_mood'):
            scene.face_mood = chosen['mood']
            scene.face_eyes = chosen['eyes']
            scene.face_mouth = chosen['mouth']
        else:
            flat = ", ".join(x for x in (chosen['mood'], chosen['eyes'], chosen['mouth']) if x)
            if flat:
                scene.expression = flat

    @staticmethod
    def _read_tags(path) -> list:
        try:
    ...
    def _get_slot_index(self) -> dict:
        """Ленивый индекс {slot: [tags]} для позы и лица из prompt-library."""
        if self._slot_index is not None:
            return self._slot_index
        idx = {}
        base = self.library.library_path
        for fname, slot in self._POSE_FILE2SLOT.items():
            idx[slot] = self._read_tags(base / '03_pose' / f'{fname}.txt')
        for fname, slot in self._FACE_FILE2SLOT.items():
            idx[slot] = self._read_tags(base / '05_expression' / f'{fname}.txt')
    ...
    def build_scene(self, location_id: str, forced_action: str | None = None) -> Scene:
        ...
        # Режим характера для этой сцены (биполярность → сэмпл по weight)
        mode = self.personality.sample_mode(random)
        ...
        if forced_action and self.force_deficit_closure:
            ...
            if forced_action and forced_action in valid_actions:
                ...
                # Применяем веса, avoid_actions и ВЕСА ХАРАКТЕРА (фаза 2)
                action_weights = self.generation_weights.get('action', {})
                if action_weights or avoid_actions or not self._personality_neutral:
                    ...
                    for action in valid_actions:
                        ...
                        if action in avoid_actions:
                            base_weight *= 0.1
                        # Вес характера (prefer/avoid по оси action); =1.0 если neutral
                        base_weight *= self._char_weight(mode, 'action', action)
        ...
        # ── Лицо и поза (фаза 2): характер взвешивает слоты; поза выбирается всегда ──
        _prefers_expr = list(set(action_soft.get("prefers_expressions", []) + soft.get("prefers_expressions", [])))
        _avoid_expr = list(set(action_soft.get("avoid_expressions", []) + soft.get("avoid_expressions", [])))
        _prefers_poses = list(set(action_soft.get("prefers_poses", []) + soft.get("prefers_poses", [])))
        _avoid_poses = list(set(action_soft.get("avoid_poses", []) + soft.get("avoid_poses", [])))
        _slot_index = self._get_slot_index()
        self._apply_face(scene, mode, _slot_index, _prefers_expr, _avoid_expr, self._personality_neutral)
        self._apply_pose(scene, mode, _slot_index, _prefers_poses, _avoid_poses)
        props = []
        props = []
