...
class SettingsTab(QWidget):
    """Вкладка настроек приложения (Qt версия)"""
    ...
    def _setup_ui(self):
        """Создаёт интерфейс вкладки Settings"""
        # Главный layout с прокруткой
        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(15, 15, 15, 15)
        
        # Scroll Area (чтобы настройки можно было скроллить, если окно маленькое)
        scroll = QScrollArea()
        ...
        browse_profiles_btn = QPushButton("Browse")
        ...
        # === СЕКЦИЯ 2: GENERATION DEFAULTS ===
        gen_group = QGroupBox("⚙️ Generation Defaults")
    ...
    def _load_settings_to_ui(self):
        """Загружает текущие настройки из SettingsManager в виджеты"""
        # Directories
    ...
    def _save_balance_var(self, key: str, checkbox: QCheckBox):
        ...
        self._log(f"⚖️ {key}: {'ON' if value else 'OFF'}\n")
    ...
    def set_output_directory(self, path):
        """Обновляет поле Output Directory (вызывается из MainWindow)"""
        self.output_dir_entry.setText(str(path))

    def set_num_scenes(self, value):
        """Обновляет поле Number of Scenes (вызывается из MainWindow)"""
        self.num_scenes_entry.setText(str(value))

    def set_balance_var(self, key: str, value):
        """Обновляет состояние чекбокса балансировки (вызывается из MainWindow)"""
        var_map = {
