    ...
    def run(self):
        ...
        try:
            ...
            if self.balance_folder:
                self.log_message.emit(f"Analyzing balance folder: {self.balance_folder}\n")
...
class GenerateTab(QWidget):
    ...
    def __init__(self, project_root: Path, profiles_directory: Path,
                 output_directory: Path, settings_manager, log_callback, parent=None):
    ...
    def _setup_ui(self):
        ...
        # Coverage Engine
        cov_group = QGroupBox("Coverage Engine")
        ...
        browse_balance_btn = QPushButton(f"{IconManager.get('folder')} Browse")
        ...
        roll_dice_btn = QPushButton(f"{IconManager.get('dice')} Roll Dice")
        roll_dice_btn.clicked.connect(self._roll_dice)
        buttons_layout.addWidget(roll_dice_btn)
        generate_btn = QPushButton(f"{IconManager.get('rocket')} Generate Batch")
        ...
        copy_log_btn = QPushButton(f"{IconManager.get('copy')} Copy All")
        copy_log_btn.setProperty("variant", "ghost")
        ...
        self._append_log("Dataset Composer ready.\n")
    ...
    def _show_force_closure_help(self):
        QMessageBox.information(
            ...
            "This mode changes the generation order:\n\n"
            "NATURAL Mode: location first, then action.\n"
            "AGGRESSIVE Mode: deficit action first, then compatible location."
        )
